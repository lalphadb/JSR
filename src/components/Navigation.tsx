        ...
        <div className="flex justify-between items-center h-20">
          ...
          <Link to="/" className="flex items-center group relative overflow-hidden">
            <div className="relative logo-transparent-glow">
              ...
              <div className="absolute -inset-4 bg-gradient-to-r from-primary/30 via-red-500/20 to-primary/30 rounded-2xl opacity-0 group-hover:opacity-100 transition-all duration-700 blur-xl animate-pulse"></div>
              ...
              <div className="absolute -inset-8 border border-red-500/15 rounded-full opacity-0 group-hover:opacity-30 transition-all duration-1000" style={{ animation: 'spin 15s linear infinite reverse' }}></div>
            ...
            <div className="absolute -top-2 -right-4 bg-gradient-to-r from-primary via-red-500 to-red-600 text-white text-xs px-3 py-1.5 rounded-full font-bold opacity-0 group-hover:opacity-100 transition-all duration-300 shadow-xl border border-white/20">
          ...
          <nav className="hidden lg:flex items-center">
              ...
              {navLinks.map((link, index) => (
                <li key={link.path}>
                  ...
                  >
                    ...
                    <span className={`absolute bottom-0 left-1/2 transform -translate-x-1/2 h-0.5 bg-gradient-to-r from-primary to-red-500 transition-all duration-300 ${
                      isActive(link.path) ? "w-3/4" : "w-0 group-hover:w-1/2"
                    }`} />
                    
                    {/* Effet de survol subtil */}
                    <div className="absolute inset-0 bg-gradient-to-r from-primary/5 to-red-500/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-lg"></div>
                  </Link>
                </li>
              ))}
            </ul>
            ...
            <Button 
              asChild 
              size="lg" 
              className="bg-gradient-to-r from-primary to-red-600 hover:from-primary/90 hover:to-red-600/90 text-white font-bold px-6 py-3 shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-105 relative overflow-hidden group"
            >
        ...
        {isOpen && (
          <div className="lg:hidden pb-6 pt-4 border-t border-slate-700/50 animate-fade-in">
              ...
              {navLinks.map((link) => (
                <Link
                  ...
                  className={`block py-4 px-6 text-lg font-semibold rounded-xl transition-all duration-300 relative group ${
                    isActive(link.path) 
                      ? "text-primary bg-gradient-to-r from-primary/10 to-red-500/10 shadow-lg border border-primary/20" 
                      : "text-slate-200 hover:text-white hover:bg-slate-700/50"
                  }`}
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-gradient-to-r from-primary/5 to-red-500/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-xl"></div>
                  
                  {/* Indicateur actif */}
                  {isActive(link.path) && (
                    <div className="absolute left-2 top-1/2 transform -translate-y-1/2 w-1 h-8 bg-gradient-to-b from-primary to-red-500 rounded-full"></div>
                  )}
                </Link>
              ))}
            </nav>
            ...
            <div className="mt-8 px-2">
              <Button asChild className="w-full bg-gradient-to-r from-primary to-red-600 hover:from-primary/90 hover:to-red-600/90 text-white font-bold py-4 shadow-xl hover:shadow-2xl transition-all duration-300 relative overflow-hidden group" size="lg">
